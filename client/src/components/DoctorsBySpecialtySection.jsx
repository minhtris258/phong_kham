...
export default function DoctorsBySpecialtySection() {
  ...
  useEffect(() => {
    const fetchSpecialties = async () => {
      try {
        ...
        else if (Array.isArray(raw?.data?.specialties)) list = raw.data.specialties;
  ...
  // ====== LẤY BÁC SĨ THEO KHOA (Giữ nguyên) ======
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        if (selectedSpecialtyId !== "ALL") params.specialty = selectedSpecialtyId;
  ...
  const displayedDoctors = useMemo(() => doctors.slice(0, 6), [doctors]);
  ...
  return (
    <section className="w-full bg-white py-12 md:py-16">
      <div className="container mx-auto px-4">
          ...
          <div className=" text-center">
            ...
            <div className="justify-center flex mb-4">
              <p className="text-sm md:text-base text-slate-600 mt-2 max-w-xl justify-center">
                Chọn chuyên khoa để xem nhanh một số bác sĩ tiêu biểu. Nhấn &quot;Xem thêm&quot; để tới trang danh sách đầy đủ.
              </p>
            </div>
          </div>
          <div className="w-full md:w-auto overflow-x-auto">
            <div className="flex gap-2 md:gap-3 min-w-max md:min-w-0">
              {loadingSpecialties && <span className="text-sm text-slate-500">Đang tải chuyên khoa...</span>}
              {!loadingSpecialties && safeSpecialties.map((spec) => {
                  const id = spec.id;
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 md:px-4 py-1.5 md:py-2 rounded-full text-sm md:text-base border transition whitespace-nowrap ${isActive ? "btn-color border-blue-400 text-white shadow-sm" : "bg-white border-slate-200 text-slate-700 hover:border-blue-500 hover:text-blue-600"}`}
                    >
                      {spec.name}
                    </button>
                  );
        ...
        {loadingDoctors ? (
          ...
          <p className="text-center text-slate-500">Chưa có bác sĩ nào cho chuyên khoa này.</p>
        ) : (
          <div className="grid md:grid-cols-3 xl:grid-cols-3 gap-6">
            {displayedDoctors.map((doctor) => {
              const {
                _id, id, fullName, thumbnail, specialty_id, consultation_fee, averageRating, address,
                // 2. Lấy thêm trường experience (hoặc yearsOfExperience tùy database của bạn)
                experience: { value: expValue } = { value: 0 }
              } = doctor;

              // Logic lấy số năm kinh nghiệm (ưu tiên field nào có dữ liệu)
    ...
    : "chưa cập nhật";
        ...
        <div className="mt-8 flex justify-center">
          <Link to="/doctors" className="inline-flex items-center px-5 py-2.5 rounded-full btn-color text-white text-sm font-semibold shadow transition">
            Xem thêm bác sĩ
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4 h-4 ml-2" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M5 12h14" />
