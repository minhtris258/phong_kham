  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-96 flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex items-end justify-between gap-2 sm:gap-4 relative px-2">
        ...
        {/* Các cột biểu đồ */}
        {!data || data.length === 0 ? (
          <div className="w-full text-center text-gray-400 text-sm mt-10 z-20">
            Không có dữ liệu
          </div>
        ) : (
          data.map((d, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Bar */}
                <div
                  className="w-full max-w-[40px] rounded-t-lg bg-blue opacity-80 group-hover:opacity-100 group-hover:from-sky-600 group-hover:to-purple-600 transition-all duration-500 ease-out shadow-sm cursor-pointer"
