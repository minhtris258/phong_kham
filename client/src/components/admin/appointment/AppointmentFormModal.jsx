  ...
  return (
    <div className="relative mb-4" ref={wrapperRef}>
      ...
      <div
        ...
        className={`w-full rounded-xl border border-gray-300 shadow-sm p-3 bg-white flex justify-between items-center cursor-pointer ${
          disabled ? "bg-gray-100 cursor-not-allowed" : "hover:border-sky-500"
        }`}
      >
        <span
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => (
                <div
                  ...
                  className={`px-4 py-3 text-sm cursor-pointer hover:bg-sky-50 transition border-b border-gray-50 last:border-0 ${
                    value === opt._id
      ...
      try {
        ...
        if (isEditing && editingAppointment.start) {
          ...
          if (!slots.find((s) => s.start === currentSlot.start)) {
            slots = [currentSlot, ...slots].sort((a, b) =>
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {isLoadingSlots ? (
            <div className="p-3 text-sm text-gray-500 bg-gray-50 rounded-xl border animate-pulse">
              Đang tải lịch rảnh...
            </div>
          ) : slotError ? (
        ...
        <div className="mt-6 flex justify-end space-x-3 pt-4 border-t border-gray-100">
          ...
          <button
            type="submit"
            className="px-6 py-2.5 bg-sky-500 text-white rounded-xl shadow-md hover:bg-sky-700 transition font-medium"
          >
