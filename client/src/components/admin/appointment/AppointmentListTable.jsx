...
const AppointmentListTable = ({
    ...
    const sortedData = useMemo(() => {
        ...
        data.sort((a, b) => {
            ...
            const timeA = new Date(`${a.date ? new Date(a.date).toISOString().split('T')[0] : ''}T${a.start}`);
            const timeB = new Date(`${b.date ? new Date(b.date).toISOString().split('T')[0] : ''}T${b.start}`);
            return timeA - timeB;
        });
        return data;
    }, [appointments]);
        ...
        <div className="bg-white p-6 rounded-xl shadow-2xl border border-gray-100 h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4 shrink-0 bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <div className="md:col-span-5 relative">
                    ...
                    <input
                        type="text"
                        className="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm"
                ...
                <div className="md:col-span-3 relative">
                    ...
                    <select
                        ...
                        className="block w-full pl-10 pr-8 py-2.5 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm cursor-pointer appearance-none"
                    >
                ...
                <div className="overflow-y-auto custom-scrollbar max-h-[550px]">
                    <table className="min-w-full divide-y divide-gray-200 relative">
                        <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm">
                            <tr>
                                <th className="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider w-16">STT</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Thông Tin</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Thời Gian</th>
                                <th className="hidden lg:table-cell px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Lý Do</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-20 text-center text-gray-500 font-medium bg-gray-50">
