...
const DoctorScheduleAdminModal = ({
  ...
  const handleDateClick = (day) => {
    ...
    setExceptionFormData({
      ...
      reason: ex?.reason || "", // <--- LẤY LÝ DO TỪ DATA CŨ
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      reason: exceptionFormData.reason, // <--- GỬI LÝ DO
    };

    try {
      await doctorSchedulesService.adminUpsertDoctorException(
        doctorId,
        payload
      );
  ...
  return (
    <Modal
      title={`Lịch làm việc - ${doctorName || ""}`}
      isOpen={isOpen}
      onClose={onClose}
      maxWidth="5xl"
    >
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {(hasDefault || ex) && (
                          <span
                            className={`absolute bottom-1 right-1 w-3 h-3 ${dot} rounded-full shadow-md`}
                          />
                        )}
                      </div>
                    );
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {!exceptionFormData.isDayOff && (
                      <>
                        ...
                        {(exceptionFormData.add.length > 0 ||
                          exceptionFormData.removeSlot.length > 0) && (
                          <div className="space-y-3 text-sm">
                            {exceptionFormData.add.map((t, i) => (
                              <div
                                key={i}
                                className="bg-green-50 text-green-700 px-4 py-2 rounded-lg flex justify-between items-center"
                              >
                                <span>
                                  {t.start} - {t.end}
                                </span>
                            ...
                            {exceptionFormData.removeSlot.map((t, i) => (
                              <div
                                key={i}
                                className="bg-red-50 text-red-700 px-4 py-2 rounded-lg flex justify-between items-center"
                              >
                                <span>
                                  {t.start} - {t.end}
                                </span>
