  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (
        ...
        !specDropdownRef.current.contains(event.target)
      ) {
        setIsSpecOpen(false);
      }
    }
                  ...
                  >
                      ...
                      <div className="relative inline-block">
                        {doc.thumbnail ? (
                          <img
                            src={doc.thumbnail}
                            alt={doc.fullName}
                            className="w-10 h-10 rounded-full object-cover border border-gray-200 shadow-sm group-hover:scale-110 transition-transform"
                          />
                        ) : (
                    ...
                    <td className="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-bold">
                      {doc.consultation_fee
                        ? doc.consultation_fee.toLocaleString("vi-VN")
                        : 0}{" "}
                      ₫
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end items-center gap-1">
                        ...
                        <button
                          onClick={() => handleChangePassword(doc)}
          ...
          <div className="hidden sm:flex flex-1 sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Trang{" "}
                <span className="font-bold text-[#00B5F1]">
                  {pagination.page}
                </span>{" "}
                / {pagination.totalPages}{" "}
                <span className="text-gray-300 mx-2">|</span> Tổng{" "}
