...
const SpecialtyDoctorsModal = ({ isOpen, onClose, currentSpecialtyDoctors, isLoading, doctorCount }) => {
    return (
        <Modal
            ...
            {isLoading ? (
                ...
                currentSpecialtyDoctors.doctors.length > 0 ? (
                    <div className="py-2 px-1 text-sm font-semibold text-gray-700 border-b border-gray-200">
                        Tổng số bác sĩ: <span className="text-indigo-600">{doctorCount}</span>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 mt-4 border border-gray-100 rounded-lg">
                            <thead className="bg-gray-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Phí Khám</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {currentSpecialtyDoctors.doctors.map(doctor => (
                                    <tr key={doctor._id || doctor.id}>
                                        ...
                                        <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500">{doctor.consultation_fee?.toLocaleString('vi-VN') || 0} VNĐ</td>
                                        <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500">{doctor.phone || 'N/A'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
