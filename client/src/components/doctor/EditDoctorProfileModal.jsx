...
export default function EditDoctorProfileModal({ doctor, onClose, onSuccess }) {
  ...
  const handleSave = async () => {
    try {
      ...
      const errorMsg =
        error.response?.data?.error || "Cập nhật thất bại. Vui lòng thử lại.";
      toastError(errorMsg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-8 relative">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all bg-gray-50 focus:bg-white"
        ...
        <div>
          ...
          <div className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  <DollarSign className="w-4 h-4 text-green-600" /> Phí khám
                  (VNĐ)
                </label>
                <input
                  type="number"
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-green-500 focus:ring-4 focus:ring-green-50 outline-none transition-all bg-gray-50 focus:bg-white"
                  placeholder="200000"
                />
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-green-600" /> Địa chỉ / Nơi
                  công tác
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  <Briefcase className="w-4 h-4 text-green-600" /> Năm bắt đầu
                  hành nghề
                </label>
                <div className="relative">
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("career_start_year", e.target.value)
                    }
                    min="1950"
                    max={currentYear}
                  ...
                  {formData.career_start_year && (
                    <span className="absolute right-3 top-3 text-sm font-semibold text-green-600 bg-green-50 px-2 rounded">
                      {Math.max(0, currentYear - formData.career_start_year)}{" "}
                      năm KN
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <FileText className="w-4 h-4 text-green-600" /> Giới thiệu kinh
                nghiệm
              </label>
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-green-500 focus:ring-4 focus:ring-green-50 outline-none transition-all bg-gray-50 focus:bg-white resize-none"
                placeholder="Mô tả chuyên sâu về kinh nghiệm, các chứng chỉ..."
              />
            </div>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 bg-white z-10 flex justify-end gap-4 pt-4 pb-2 border-t border-gray-100">
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className={`px-8 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 flex items-center gap-2 font-medium shadow-md hover:shadow-lg transition-all active:scale-95 ${
