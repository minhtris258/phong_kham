...
import { Plus, Trash2, Save, Calendar, Pill, DollarSign, FileText } from "lucide-react";
import { toast } from "react-toastify";
import visitService from "../../../services/VisitService";
import Modal from "../../Modal"; // Đảm bảo đường dẫn đúng tới file Modal.jsx của bạn

const VisitCreateModal = ({ isOpen, onClose, appointment, onSuccess }) => {
  // === STATE & LOGIC (Giữ nguyên logic cũ) ===
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data?.followup?.scheduled) {
        toast.info(`Đã tự động đặt lịch tái khám: ${res.data.followup.date}`);
      }
  ...
  // Lấy tên bệnh nhân để hiển thị trên Title Modal
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={modalTitle}
      maxWidth="5xl" // Sử dụng độ rộng lớn hơn (5xl) cho thoải mái
    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <input
                ...
                placeholder="Dặn dò ăn uống, sinh hoạt..."
              />
            </div>
            <div className="col-span-2 md:col-span-1">
            ...
            <div className="grid gap-2 p-3">
                ...
                <div key={index} className="flex flex-col md:flex-row gap-2 items-start md:items-center bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <input 
                        ...
                        className="flex-1 border-gray-300 border p-2 rounded text-sm w-full focus:border-blue-500 outline-none"
                        value={item.drug}
                        onChange={(e) => handlePrescriptionChange(index, 'drug', e.target.value)}
                    />
                    <input 
                        placeholder="Liều (VD: 1v)" 
                        className="w-full md:w-24 border-gray-300 border p-2 rounded text-sm focus:border-blue-500 outline-none"
                        value={item.dosage}
                        onChange={(e) => handlePrescriptionChange(index, 'dosage', e.target.value)}
                    />
                    <input 
                        placeholder="Sáng/Chiều" 
                        className="w-full md:w-36 border-gray-300 border p-2 rounded text-sm focus:border-blue-500 outline-none"
                        value={item.frequency}
                        onChange={(e) => handlePrescriptionChange(index, 'frequency', e.target.value)}
                    />
                    <input 
                        placeholder="Thời gian" 
                        className="w-full md:w-28 border-gray-300 border p-2 rounded text-sm focus:border-blue-500 outline-none"
                        value={item.duration}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {billItems.map((item, index) => (
                        <div key={index} className="flex gap-2 items-center">
                            <input 
                                ...
                                className="flex-1 border border-gray-300 p-2 rounded text-sm focus:border-blue-500 outline-none"
                                value={item.name}
                            ...
                            <input 
                                type="number"
                                placeholder="SL" 
                                className="w-16 border border-gray-300 p-2 rounded text-sm text-center focus:border-blue-500 outline-none"
                                value={item.quantity}
                                min="1"
                                onChange={(e) => handleBillChange(index, 'quantity', e.target.value)}
                            />
                            <input 
                                type="number"
                                placeholder="Giá" 
                                className="w-28 border border-gray-300 p-2 rounded text-sm text-right focus:border-blue-500 outline-none"
                                value={item.price}
                                min="0"
                                onChange={(e) => handleBillChange(index, 'price', e.target.value)}
                            />
            ...
            <section className="bg-indigo-50 p-5 rounded-xl border border-indigo-100 h-fit">
                ...
                <p className="text-sm text-gray-600 mb-4 leading-relaxed">
                    Hệ thống sẽ tự động tìm khung giờ trống tiếp theo của bạn vào ngày được chọn để đặt lịch.
                </p>
                
                <div className="bg-white p-1 rounded-lg border border-indigo-200 shadow-sm">
                    <input 
                        type="date"
                        ...
                        onChange={handleChange}
                        className="w-full border-none focus:ring-0 text-gray-800 font-medium p-2 rounded-md"
                    />
                </div>
                ...
                {formData.next_visit_date && (
                    <div className="mt-3 text-xs flex items-center gap-1 text-indigo-700 bg-indigo-100/50 p-2 rounded">
                        ...
                        Sẽ tạo lịch hẹn loại "Follow-up"
                    </div>
                )}
            </section>
        </div>
