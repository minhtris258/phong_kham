...
const VisitCreateModal = ({ isOpen, onClose, appointment, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // Load danh mục thuốc
      medicineService
        .getAllMedicines({ limit: 1000, status: "active" })
      ...
      medicalServiceService
        .getAllServices({ limit: 100, status: "active" })
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        const res = await timeslotService.getSlotsByDate(
          doctorId,
          formData.next_visit_date
        );
        const slots = res.data?.data || res.data || [];
        setAvailableTimeslots(
          Array.isArray(slots) ? slots.filter((s) => s.status === "free") : []
        );
      } catch (error) {
        console.error(error);
        setAvailableTimeslots([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data?.followup?.scheduled) {
        toastInfo(
          `Đã hẹn tái khám ngày: ${new Date(
            res.data.followup.date
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-1 md:col-span-2">
                ...
                <textarea
                  ...
                  rows="2"
                  className="w-full mt-1 border border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 outline-none"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows="2"
                  className="w-full mt-1 border border-gray-300 rounded-lg p-2.5 outline-none"
                  placeholder="Dặn dò ăn uống, nghỉ ngơi..."
                />
              </div>
              <div className="col-span-1 md:col-span-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-1">
                  <AlertCircle size={14} /> Ghi chú nội bộ
                </label>
                <input
                  name="notes"
                  value={formData.notes}
                  onChange={handleChange}
                  className="w-full mt-1 border border-gray-300 rounded-lg p-2.5 outline-none text-sm bg-gray-50"
                  placeholder="Ghi chú chỉ bác sĩ/admin thấy..."
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex justify-between items-center mb-4 border-b pb-2">
              ...
              <button
                ...
                className="text-sm bg-green-50 text-green-700 px-3 py-1 rounded-md font-medium hover:bg-green-100 transition flex items-center gap-1"
              >
            ...
            <div className="space-y-3">
              ...
              {prescriptions.map((item, index) => (
                <div
                  key={index}
                  className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start bg-gray-50 p-3 md:p-2 rounded-lg border border-gray-100 shadow-sm md:shadow-none"
                >
                  {/* Tên thuốc */}
                  ...
                  <div className="md:col-span-1">
                    ...
                    <input
                      placeholder="Viên"
                      className="w-full border-gray-300 border p-2 rounded text-sm outline-none text-center bg-gray-100"
                      value={item.unit}
                      readOnly
                      tabIndex={-1}
                    />
                  ...
                  <div className="md:col-span-1">
                    ...
                    <input
                      type="number"
                      min="1"
                      className="w-full border-gray-300 border p-2 rounded text-sm text-center outline-none font-bold text-green-700"
                      value={item.quantity}
                      onChange={(e) =>
                  ...
                  <div className="md:col-span-2">
                    ...
                    {item.availableDosages &&
                      item.availableDosages.length > 0 && (
                        <datalist id={`dosage-list-${index}`}>
                          {item.availableDosages.map((d, i) => (
                  ...
                  <div className="md:col-span-2">
                    ...
                    {/* Note nhỏ */}
                    <input
                      placeholder="Ghi chú (sau ăn...)"
                      className="w-full mt-1 border-gray-200 border p-1.5 rounded text-xs outline-none text-gray-600"
                      value={item.note}
                      onChange={(e) =>
                  ...
                  <div className="md:col-span-1 flex justify-center pt-2">
                    <button
                      ...
                      className="text-gray-400 hover:text-red-600 bg-white p-1 rounded-full hover:bg-red-50 transition"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                ...
                <button
                  onClick={addService}
                  className="text-sm bg-yellow-50 text-yellow-700 px-3 py-1 rounded-md font-medium hover:bg-yellow-100 transition flex items-center gap-1"
                >
                  <Plus size={16} /> Thêm
                </button>
              </div>
            ...
            {/* Tái khám */}
            <div className="bg-sky-50 p-4 rounded-xl border border-sky-100 h-fit">
              <div className="flex items-center gap-2 mb-3">
                <Calendar size={20} className="text-sky-600" />
                <h3 className="font-bold text-sky-800">Hẹn tái khám</h3>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full border border-sky-200 p-2 rounded-lg focus:ring-2 focus:ring-sky-500 outline-none"
                  />
                </div>
                  ...
                  <div className="animate-fade-in-down">
                    ...
                    {loadingSlots ? (
                      <div className="text-sm text-sky-500 flex items-center gap-2">
                        <div className="animate-spin">⏳</div> Đang tìm giờ...
                      </div>
                    ) : (
                      <div className="relative">
                        <select
                          ...
                          className="w-full border border-sky-200 p-2 rounded-lg appearance-none bg-white pr-8 cursor-pointer outline-none text-sm"
                        >
        ...
        <div className="border-t border-gray-200 pt-4 mt-2 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col">
            <span className="text-sm text-gray-500">
              Tổng chi phí dự kiến (Chưa bao gồm thuốc):
            </span>
            <span className="text-2xl font-bold text-blue-600">
              {totalAmountEstimate.toLocaleString("vi-VN")} đ
            </span>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <button
              onClick={onClose}
              disabled={loading}
              className="flex-1 md:flex-none px-6 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50"
            >
              Hủy
            </button>
            <button
              ...
              disabled={loading}
              className="flex-1 md:flex-none px-8 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 flex justify-center items-center gap-2 disabled:opacity-70 shadow-lg shadow-blue-200"
            >
              {loading ? (
