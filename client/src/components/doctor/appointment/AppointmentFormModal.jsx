...
const AppointmentFormModal = ({
    ...
    const isRescheduling = isEditing && formData.start && editingAppointment.start && formData.start !== editingAppointment.start;
    ...
    useEffect(() => {
        const fetchSlots = async () => {
            ...
            try {
                ...
                let slots = Array.isArray(res.data) ? res.data : (Array.isArray(res) ? res : []);
                ...
                const originalDate = editingAppointment?.date ? String(editingAppointment.date).substring(0, 10) : '';
    ...
    return (
        <Modal 
            title={isRescheduling ? 'Xác Nhận Dời Lịch' : (isEditing ? 'Cập Nhật Thông Tin' : 'Thêm Lịch Hẹn')} 
            isOpen={isOpen} 
            onClose={onClose}
            maxWidth="lg"
        >
                ...
                <div className="space-y-5">
                    ...
                    {isRescheduling && (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="text-sm text-yellow-700">
                                ...
                                <p>Lịch cũ: <b>{editingAppointment.start}</b> {'->'} Lịch mới: <b>{formData.start}</b></p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <label className="block">
                            ...
                            <select 
                                ...
                                className={`mt-1 block w-full rounded-xl border-gray-300 shadow-sm p-3 focus:ring-indigo-500 focus:border-indigo-500 ${isEditing ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`}
                            >
                                <option value="">-- Chọn bệnh nhân --</option>
                                {mockPatients.map(p => (
                                    <option key={p._id} value={p._id}>{p.fullName || p.name} - {p.phone}</option>
                                ))}
                            </select>
                        </label>
                        <label className="block">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <label className="block">
                            ...
                            <input 
                                ...
                                onChange={handleInputChange}
                                required
                                className="mt-1 block w-full rounded-xl border-gray-300 shadow-sm p-3 focus:ring-indigo-500 focus:border-indigo-500"
                            />
                        </label>
                        ...
                        <label className="block">
                            ...
                            {isLoadingSlots ? (
                                ...
                                <div className="mt-1 p-3 text-sm text-red-500 bg-red-50 rounded-xl border border-red-200 text-center">{slotError}</div>
                            ) : (
                                <select 
                                    name="start"
                                    ...
                                    className={`mt-1 block w-full rounded-xl border-gray-300 shadow-sm p-3 ${availableSlots.length === 0 ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white focus:ring-indigo-500 focus:border-indigo-500'}`}
                                >
                                    <option value="">
                                        {availableSlots.length === 0 ? "Hết lịch trống" : "-- Chọn giờ --"}
                                    </option>
                    ...
                    {isEditing && !isRescheduling && (
                        <label className="block">
                            ...
                            <select 
                                ...
                                onChange={handleInputChange}
                                className="mt-1 block w-full rounded-xl border-gray-300 shadow-sm p-3 bg-white focus:ring-indigo-500"
                            >
                    ...
                    <label className="block">
                        ...
                        <textarea
                            ...
                            onChange={handleInputChange}
                            rows="3"
                            placeholder={isRescheduling ? "Nhập lý do dời lịch..." : "Mô tả..."}
                            className="mt-1 block w-full rounded-xl border-gray-300 shadow-sm p-3 focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                        ></textarea>
                    </label>
                </div>
                ...
                <div className="mt-8 flex justify-end space-x-3 border-t pt-4">
                    ...
                    <button 
                        ...
                        className={`px-6 py-2.5 text-white rounded-xl shadow-lg transition font-medium flex items-center gap-2 ${
                            isRescheduling ? 'bg-orange-600 hover:bg-orange-700' : 'bg-indigo-600 hover:bg-indigo-700'
                        }`}
                    >
                        {isRescheduling ? 'Xác nhận Dời Lịch' : (isEditing ? 'Cập nhật' : 'Tạo Lịch Hẹn')}
                    </button>
                </div>
            </form>
        </Modal>
