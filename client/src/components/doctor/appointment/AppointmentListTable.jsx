...
import { 
    Plus, CalendarSync, CalendarX, ClipboardPen, 
    Filter, ChevronLeft, ChevronRight, Search,
    User, Clock, FileText, // Thêm icons cho mobile view
    CalendarIcon
} from "lucide-react";
...
const AppointmentListTable = ({
  ...
  const processedData = useMemo(() => {
    ...
    data.sort((a, b) => {
        ...
        const dateTimeA = new Date(`${String(a.date).substring(0, 10)}T${a.start}`);
        const dateTimeB = new Date(`${String(b.date).substring(0, 10)}T${b.start}`);
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <div className="relative w-full sm:w-auto">
                ...
                <input 
                    ...
                    className="w-full sm:w-auto pl-9 pr-4 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white text-gray-700 cursor-pointer hover:bg-gray-50 transition"
                />
            </div>
            ...
            <div className="relative w-full sm:w-auto">
                ...
                <select
                    ...
                    className="w-full sm:w-auto pl-9 pr-8 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white cursor-pointer hover:bg-gray-50 transition appearance-none"
                >
            ...
            <>
                ...
                <div className="md:hidden space-y-4">
                    {paginatedData.map((app) => (
                        <div key={app._id || app.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200 shadow-sm relative">
                            {/* Status Badge (Góc phải trên) */}
                            <span className={`absolute top-4 right-4 px-2 py-1 text-xs font-bold rounded-lg ${getStatusStyle(app.status)}`}>
                            ...
                            <div className="space-y-3 mb-4 pr-20">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <div className="font-bold text-gray-900 text-base">{getDisplay(app.patient_id)}</div>
                                        {app.patient_id?.phone && <div className="text-xs text-gray-500">{app.patient_id.phone}</div>}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-3 text-sm text-gray-700">
                                    ...
                                    <div>
                                        <span className="font-bold text-indigo-700 text-lg mr-2">{app.start}</span>
                                        <span className="text-gray-500 text-xs">{new Date(app.date).toLocaleDateString("vi-VN")}</span>
                                    </div>
                                </div>

                                <div className="flex items-start gap-3 text-sm text-gray-600 bg-white p-2 rounded border border-gray-100">
                                    <FileText className="w-4 h-4 text-gray-400 mt-0.5 shrink-0" />
                                    <span className="line-clamp-2 italic">{app.reason || "Không có lý do khám"}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-2 border-t border-gray-200 pt-3 mt-2">
                                {["confirmed", "pending"].includes(app.status) && (
                                    <button onClick={() => handleOpenVisitModal(app)} className="flex items-center gap-1 text-green-700 bg-green-50 hover:bg-green-100 px-3 py-2 rounded-lg text-sm font-medium transition flex-1 justify-center">
                                        <ClipboardPen className="w-4 h-4" /> Khám
                                    </button>
                                )}
                                {app.status !== "cancelled" && app.status !== "completed" && (
                                    <button onClick={() => handleAddEdit(app)} className="flex items-center gap-1 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-3 py-2 rounded-lg text-sm font-medium transition flex-1 justify-center">
                                        <CalendarSync className="w-4 h-4" /> Sửa
                                    </button>
                                )}
                                {app.status !== "cancelled" && app.status !== "completed" && (
                                    <button onClick={() => confirmCancel(app._id || app.id)} className="flex items-center gap-1 text-red-700 bg-red-50 hover:bg-red-100 px-3 py-2 rounded-lg text-sm font-medium transition flex-1 justify-center">
                ...
                <div className="hidden md:block overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-100 sticky top-0 z-10">
                            ...
                            <th className="hidden lg:table-cell px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Lý Do</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            <tr key={app._id || app.id} className="hover:bg-indigo-50/50 transition duration-150 group">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    <div className="flex flex-col">
                                        ...
                                        {app.patient_id?.phone && <span className="text-xs text-gray-500">{app.patient_id.phone}</span>}
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                                    <div className="flex items-center gap-2">
                                        <span className="text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded">{app.start}</span>
                                        <span className="text-xs text-gray-500">{new Date(app.date).toLocaleDateString("vi-VN", {day: '2-digit', month: '2-digit'})}</span>
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex justify-end items-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                                        {["confirmed", "pending"].includes(app.status) && (
                                            <button onClick={() => handleOpenVisitModal(app)} className="text-green-600 hover:text-green-900 p-2 rounded-full hover:bg-green-100 transition" title="Khám bệnh">
                                                <ClipboardPen className="w-5 h-5" />
                                            </button>
                                        )}
                                        {app.status !== "cancelled" && app.status !== "completed" && (
                                            <button onClick={() => handleAddEdit(app)} className="text-indigo-600 hover:text-indigo-900 p-2 rounded-full hover:bg-indigo-100 transition" title="Sửa">
                                                <CalendarSync className="w-5 h-5" />
                                            </button>
                                        )}
                                        {app.status !== "cancelled" && app.status !== "completed" ? (
                                            <button onClick={() => confirmCancel(app._id || app.id)} className="text-red-600 hover:text-red-900 p-2 rounded-full hover:bg-red-100 transition" title="Hủy">
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 pt-4 mt-4 gap-4">
            <div className="text-sm text-gray-500 text-center sm:text-left">
                Trang <span className="font-medium text-indigo-600">{currentPage}</span> / {totalPages} 
                <span className="hidden sm:inline"> • Tổng {processedData.length} lịch hẹn</span>
            </div>
            <div className="flex items-center gap-2">
                <button
                ...
                <span className="sm:hidden font-medium text-gray-700 px-2">{currentPage}</span>
