...
export default function ContactManagement() {
  ...
  const handleSendReply = async () => {
    if (!replyMessage.trim()) return toastError("Vui lòng nhập nội dung phản hồi");
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "new":
        return <span className="px-2 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-bold border border-blue-100 flex items-center gap-1 w-fit"><AlertCircle size={12}/> Mới</span>;
      case "in_progress":
        return <span className="px-2 py-1 rounded-full bg-yellow-50 text-yellow-600 text-xs font-bold border border-yellow-100 flex items-center gap-1 w-fit"><Clock size={12}/> Đang xử lý</span>;
      case "resolved":
        return <span className="px-2 py-1 rounded-full bg-green-50 text-green-600 text-xs font-bold border border-green-100 flex items-center gap-1 w-fit"><CheckCircle size={12}/> Đã xong</span>;
      default:
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold text-slate-800">Quản lý Liên hệ & Phản hồi</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="p-8 text-center text-slate-500">Không có liên hệ nào.</td></tr>
              ) : (
                contacts.map((contact) => (
                  <tr key={contact._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 max-w-xs">
                      <p className="font-semibold text-sky-700 text-sm truncate">{contact.subject}</p>
                      <p className="text-xs text-slate-500 line-clamp-2 mt-1">{contact.message}</p>
                    </td>
                    <td className="p-4 text-sm text-slate-600">
                      {new Date(contact.createdAt).toLocaleDateString("vi-VN")}
                      <br/>
                      <span className="text-xs text-slate-400">{new Date(contact.createdAt).toLocaleTimeString("vi-VN")}</span>
                    </td>
                    <td className="p-4">
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-200 flex justify-center gap-2">
            ...
            <span className="px-3 py-1 text-sm font-semibold text-slate-700">Trang {page} / {totalPages}</span>
            <button 
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fade-in">
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-start">
              <div>
                <h2 className="text-xl font-bold text-slate-800">Chi tiết Liên hệ</h2>
                <div className="mt-2 flex gap-3">
                   {getStatusBadge(selectedContact.status)}
                   <span className="text-sm text-slate-500">{new Date(selectedContact.createdAt).toLocaleString("vi-VN")}</span>
                </div>
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase">Điện thoại</label>
                  <p className="text-slate-800 font-medium">{selectedContact.phone || "---"}</p>
                </div>
                <div className="col-span-2">
                  <label className="text-xs font-bold text-slate-500 uppercase">Email</label>
                  <p className="text-sky-600 font-medium">{selectedContact.email}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="font-bold text-slate-800 mb-3">{selectedContact.subject}</p>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => handleStatusChange(selectedContact._id, "in_progress")}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border ${selectedContact.status === 'in_progress' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                  >
                    Đang xử lý
                  </button>
                  <button 
                    onClick={() => handleStatusChange(selectedContact._id, "resolved")}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border ${selectedContact.status === 'resolved' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                  >
                    Đã giải quyết
                  </button>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <p className="text-xs text-slate-400 mt-2 text-right italic">
                  * Khi gửi phản hồi, trạng thái sẽ tự động chuyển thành "Đã xong".
                </p>
              </div>
