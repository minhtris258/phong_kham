...
const AppointmentManagement = () => {
    ...
    const getDoctorName = useCallback((id) => doctorMap.get(id)?.fullName || `BS ID: ${id}`, [doctorMap]);
    const getPatientName = useCallback((id) => patientMap.get(id)?.fullName || `BN ID: ${id}`, [patientMap]);
    ...
    const checkAvailability = useCallback((doctorId, date, startTime, excludeAppId = null) => {
...
const confirmDelete = (id) => {
    ...
    const handleAddEdit = useCallback((appointment) => {
        ...
        setFormData(appointment ? 
            ...
            { patient_id: defaultPatientId, doctor_id: defaultDoctorId, date: defaultDate, start: '09:00', status: 'pending', reason: '' }
        );
        setIsModalOpen(true);
    }, [currentDayDetails]);
    ...
    const handleSave = (newFormData) => {
        ...
        if (isEditing) {
            ...
            const newApp = { ...newFormData, id: 'mock-a-' + Math.random().toString(36).substring(7), created_at: new Date().toISOString() };
        ...
        if (currentDayDetails.date === newFormData.date) {
            setCurrentDayDetails(prev => ({
                ...prev,
                apps: updatedAppointments.filter(app => app.date === newFormData.date)
            }));
        }
    };
    ...
    // === 7. HÀM XỬ LÝ XÓA ===
    const handleDelete = () => {
        const updatedAppointments = appointments.filter(app => app.id !== confirmDeleteId);
        setAppointments(updatedAppointments);
        ...
        <main className="flex-1 p-4 sm:p-8 bg-gray-50 min-h-[calc(100vh-64px)]">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex space-x-2 p-1 bg-white rounded-xl shadow-md border border-gray-200">
                    <button 
                        onClick={() => { setViewMode('calendar'); setCurrentDayDetails({ date: null, apps: [] }); }}
                        className={`flex items-center px-4 py-2 rounded-lg text-sm font-semibold transition ${viewMode === 'calendar' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                        <Calendar className="w-5 h-5 mr-2" /> Chế Độ Lịch
                    </button>
                    <button 
                        onClick={() => { setViewMode('list'); setCurrentDayDetails({ date: null, apps: [] }); }}
                        className={`flex items-center px-4 py-2 rounded-lg text-sm font-semibold transition ${viewMode === 'list' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                ...
                {viewMode === 'list' && (
                    <div className="lg:col-span-3">
                        ...
                        <div className="mb-4 flex items-center space-x-3 bg-white p-4 rounded-xl shadow-md border">
                            <label className="text-sm font-semibold text-gray-700">Lọc theo ngày:</label>
                            <input 
                                type="date" 
