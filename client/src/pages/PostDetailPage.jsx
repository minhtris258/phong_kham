...
const BlockRenderer = ({ block }) => {
  switch (block.type) {
    case "heading":
      ...
      const headingClass =
        block.level === 1
          ? "text-3xl md:text-4xl font-bold text-slate-900 mt-10 mb-6"
          : block.level === 2
          ? "text-2xl md:text-3xl font-bold text-slate-800 mt-10 mb-5 pb-2 border-b border-slate-100"
    ...
    case "quote":
      return (
        <blockquote className="border-l-4 border-sky-500 pl-6 italic text-slate-600 text-xl my-10 py-4 bg-sky-50/50 rounded-r-lg">
          "{block.text}"
        </blockquote>
      );
    ...
    case "list":
      ...
      return (
        <ListTag
          className={`${listClass} list-outside space-y-3 mb-8 ml-6 text-slate-700 text-lg leading-8`}
        >
  ...
  if (error || !post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 px-4 pt-20">
        ...
        <Link
          to="/post"
          className="px-6 py-2 bg-sky-600 text-white rounded-full hover:bg-sky-700 transition flex items-center gap-2"
        >
        ...
        <article className="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden">
          ...
          <div className="relative w-full aspect-video md:aspect-[21/9] bg-slate-100 group">
            ...
            <button
              ...
              className="absolute top-4 left-4 z-10 flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-md rounded-full shadow-md text-slate-700 text-sm font-semibold hover:bg-white hover:text-sky-600 transition-all duration-300 border border-white/50 cursor-pointer"
            >
              <ArrowLeft size={18} />
          ...
          <div className="px-6 py-8 md:px-12 md:py-12">
              ...
              <div className="flex flex-wrap gap-2">
                {post.tags &&
                  post.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center px-3 py-1 bg-sky-50 text-sky-700 text-xs font-bold rounded-full uppercase tracking-wide border border-sky-100"
                    >
            ...
            <div className="mt-16 pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  className="p-2 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-200 transition"
                  title="Copy Link"
                >
                  <Share2 size={18} />
                </button>
        ...
        {relatedPosts.length > 0 && (
          <div className="mt-16 mb-10">
            <div className="flex items-center justify-between mb-8 px-2">
              ...
              <Link
                to="/post"
                className="text-sky-500 hover:text-sky-600 text-sm font-semibold flex items-center group"
              >
                Xem tất cả{" "}
                <ArrowLeft
                  size={16}
                  className="rotate-180 ml-1 group-hover:translate-x-1 transition-transform"
                />
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedPosts.map((item) => (
                <Link
                  to={`/post/${item.slug}`}
                  key={item._id}
                  className="group flex flex-col bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg transition-all duration-300"
                >
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <span className="title-color text-sm font-semibold mt-auto flex items-center">
                      ...
                      <ArrowLeft
                        size={14}
                        className="rotate-180 ml-1 opacity-0 group-hover:opacity-100 transition-opacity"
                      />
                    </span>
                  </div>
                </Link>
