...
export default function ServicesPage() {
  ...
  useEffect(() => { setCurrentPage(1); }, [selectedPriceRange, sortOption]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 mt-14 py-10 min-h-screen">
      ...
      <div className="text-center mb-10">
        <h1 className="text-3xl lg:text-4xl font-bold text-[#0a2463]">DỊCH VỤ CHUYÊN SÂU</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <aside className="lg:col-span-3">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sticky top-24">
             ...
             <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-[#0a2463] font-bold text-lg"><Filter size={20} /> Bộ lọc</div>
              <button onClick={() => { setSelectedPriceRange("ALL"); setSortOption("default"); }} className="text-xs font-semibold text-red-500 hover:underline">Đặt lại</button>
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-700 mb-3 uppercase">Khoảng giá</h3>
              <div className="space-y-3">
                {[{ label: "Tất cả", value: "ALL" }, { label: "< 200k", value: "<200" }, { label: "200k - 500k", value: "200-500" }, { label: "> 500k", value: ">500" }].map((option) => (
                  <label key={option.value} className="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="price" value={option.value} checked={selectedPriceRange === option.value} onChange={(e) => setSelectedPriceRange(e.target.value)} className="peer h-4 w-4 cursor-pointer" />
                    <span className="text-sm peer-checked:font-bold peer-checked:text-blue-600">{option.label}</span>
                  </label>
                ))}
              </div>
            </div>
        ...
        <main className="lg:col-span-9">
          ...
          <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
            <p className="text-slate-600 text-sm">Hiển thị <b>{processedServices.length}</b> dịch vụ</p>
            <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className="bg-slate-50 border border-slate-200 text-sm rounded-lg p-2">
          ...
          {totalPages > 1 && (
            <div className="mt-10 flex justify-center gap-2">
              <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="w-10 h-10 border rounded-lg hover:bg-slate-50 disabled:opacity-50"><ChevronLeft /></button>
              <span className="flex items-center font-bold px-2">Trang {currentPage}</span>
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="w-10 h-10 border rounded-lg hover:bg-slate-50 disabled:opacity-50"><ChevronRight /></button>
            </div>
          )}
        </main>
      </div>
