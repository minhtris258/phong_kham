...
export default function DoctorSchedule() {
  ...
  const { startDayIndex, daysInMonth } = useMemo(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      toastError(
        "Lỗi: " + (err.response?.data?.error || "Không thể lưu thay đổi")
      );
    }
  };
  ...
  if (loading && !scheduleData) {
    return (
      <div className="flex justify-center items-center h-screen text-sky-600">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Xem và điều chỉnh lịch khám của bạn
          </p>
        </div>
        <button
          onClick={() => setIsConfigModalOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-sky-500 text-white rounded-xl hover:bg-sky-700 shadow-lg shadow-sky-200 transition font-medium"
        >
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="grid grid-cols-7 divide-x divide-gray-200 min-h-[250px]">
              {daysOfWeek.map((d) => {
                const sch = scheduleData?.weekly_schedule?.find(
                  (s) => s.dayOfWeek === dayEnMap[d]
                );
                return (
                  <div
                    key={d}
                    className="flex flex-col items-center py-6 px-2 hover:bg-gray-50 transition-colors"
                  >
                    {/* Header Ngày (T2, T3...) */}
                    <div className="text-xl font-bold text-sky-600 mb-6">
                      {d}
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                const dayOfWeekEn = dateObj.toLocaleString("en-US", {
                  weekday: "long",
                });

                const hasDefault = scheduleData?.weekly_schedule?.some(
                  (s) => s.dayOfWeek === dayOfWeekEn && s.timeRanges?.length > 0
                );
                ...
                return (
                  <div
                    ...
                    className={`
                                            ...
                                            ${
                                              isSelected
                                                ? "ring-2 ring-sky-600 shadow-md transform scale-105 z-10 bg-white"
                                                : ""
                                            }
                                            ${
                                              isToday && !isSelected
            ...
            <div className="sticky top-8 bg-white rounded-2xl shadow-xl border border-gray-100 p-6 animate-fade-in-up">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-7 w-12 items-center rounded-full transition ${
                      exceptionFormData.isDayOff ? "bg-red-500" : "bg-gray-300"
                    }`}
                  >
                    <span
                ...
                <div>
                  ...
                  <textarea
                    rows={2}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm"
                  ...
                  <>
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (
                              tempTimeInput.start &&
                              tempTimeInput.end &&
                              tempTimeInput.start < tempTimeInput.end
                            ) {
                              setExceptionFormData((prev) => ({
                                ...prev,
                                add: [...prev.add, tempTimeInput],
                              }));
                              setTempTimeInput({ start: "", end: "" });
                            } else {
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (
                              ...
                              setExceptionFormData((prev) => ({
                                ...prev,
                                removeSlot: [...prev.removeSlot, tempTimeInput],
                              }));
                              setTempTimeInput({ start: "", end: "" });
                            } else {
                              toastWarning("Giờ không hợp lệ");
                    ...
                    <div className="space-y-2 mt-4 max-h-40 overflow-y-auto">
                      {exceptionFormData.add.map((t, i) => (
                        <div
                          key={`add-${i}`}
                          className="flex justify-between items-center bg-green-50 text-green-700 p-2 rounded text-sm border border-green-100"
                        >
                          <span className="flex items-center gap-2">
                            <Plus className="w-3 h-3" /> {t.start} - {t.end}
                          </span>
                          <Trash2
                            onClick={() =>
                              setExceptionFormData((prev) => ({
                                ...prev,
                                add: prev.add.filter((_, idx) => idx !== i),
                              }))
                            }
                      ...
                      {exceptionFormData.removeSlot.map((t, i) => (
                        <div
                          key={`rem-${i}`}
                          className="flex justify-between items-center bg-orange-50 text-orange-700 p-2 rounded text-sm border border-orange-100"
                        >
                          <span className="flex items-center gap-2">
                            <X className="w-3 h-3" /> {t.start} - {t.end}
                          </span>
                          <Trash2
                            onClick={() =>
                              setExceptionFormData((prev) => ({
                                ...prev,
                                removeSlot: prev.removeSlot.filter(
                                  (_, idx) => idx !== i
                                ),
                              }))
                            }
                ...
                <div className="flex gap-3 pt-4 border-t">
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-2.5 bg-sky-600 text-white rounded-lg hover:bg-sky-700 shadow-md font-medium text-sm transition transform active:scale-95"
                  >
                    Lưu thay đổi
                  </button>
                </div>
