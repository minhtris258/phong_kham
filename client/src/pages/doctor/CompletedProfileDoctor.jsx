...
const CompletedProfileDoctor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // === LOGIC CẬP NHẬT TOKEN MỚI ===
      // Kiểm tra token ở res.token hoặc res.data.token
      const newToken = res.token || res.data?.token;

      if (newToken) {
          console.log("Nhận token mới, đang cập nhật...");
      ...
      <div className="w-full max-w-6xl bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
        <div className="grid lg:grid-cols-5 min-h-[600px]">
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-8 lg:col-span-2 flex flex-col justify-between relative overflow-hidden">
             {/* ... (Giữ nguyên phần UI trang trí) ... */}
             <div className="relative z-10">
              <p className="text-xs font-bold uppercase tracking-widest text-blue-200 mb-4">Bước cuối cùng</p>
              <h1 className="text-3xl font-bold leading-tight mb-4">Xây dựng hồ sơ chuyên nghiệp</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Nên dùng ảnh chân dung rõ mặt, mặc áo blouse.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="md:col-span-2">
                  ...
                  <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" required />
                </div>
                <div>
                   <label className="block text-sm font-semibold text-slate-700 mb-1.5">Số điện thoại <span className="text-red-500">*</span></label>
                   <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" required />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Ngày sinh <span className="text-red-500">*</span></label>
                  <input type="date" name="dob" value={formData.dob} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" required />
                </div>
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Giới tính</label>
                    <div className="flex gap-4">
                        {["male", "female", "other"].map((g) => (
                            <label key={g} className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 border rounded-lg cursor-pointer ${formData.gender === g ? "bg-indigo-50 border-indigo-500 text-indigo-700" : "border-slate-200"}`}>
              ...
              <div className="border-t border-slate-100 pt-6">
                 <h3 className="text-lg font-bold text-slate-800 mb-4">Thông tin Chuyên môn</h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="md:col-span-2">
                        <label className="block text-sm font-semibold text-slate-700 mb-1.5">Chuyên khoa <span className="text-red-500">*</span></label>
                        <select name="specialty_id" value={formData.specialty_id} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="consultation_fee" value={formData.consultation_fee} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" required min="0"/>
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1.5">Năm bắt đầu</label>
                        <input type="number" name="career_start_year" value={formData.career_start_year} onChange={handleChange} className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border" />
                    </div>
                 </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-6">
                 <div className="mb-4">
                     <label className="block text-sm font-semibold text-slate-700 mb-1.5">Địa chỉ làm việc</label>
                     <input type="text" name="address" value={formData.address} onChange={handleChange} className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm" required />
                 </div>
                 <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Giới thiệu (Bio)</label>
                    <textarea name="introduction" value={formData.introduction} onChange={handleChange} rows="4" className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm border"></textarea>
                </div>
              </div>

              {/* BUTTON */}
              <div className="pt-4 flex items-center justify-end">
                <button type="submit" disabled={loading} className={`inline-flex items-center justify-center rounded-xl px-8 py-3.5 text-sm font-semibold text-white shadow-lg transition-all ${loading ? "bg-indigo-400" : "bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700"}`}>
