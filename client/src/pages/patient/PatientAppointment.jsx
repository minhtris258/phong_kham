...
import { 
    Calendar, Clock, User, AlertCircle, CheckCircle, XCircle, FileText, 
...
const PatientAppointment = () => {
    ...
    const getStatusBadge = (status) => {
        switch (status?.toLowerCase()) {
            case 'confirmed':
            case 'approved':
                return <span className="flex items-center gap-1 text-green-600 bg-green-50 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap"><CheckCircle size={14} /> Đã xác nhận</span>;
            case 'pending':
                return <span className="flex items-center gap-1 text-yellow-600 bg-yellow-50 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap"><Clock size={14} /> Chờ xác nhận</span>;
            case 'cancelled':
                return <span className="flex items-center gap-1 text-red-600 bg-red-50 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap"><XCircle size={14} /> Đã hủy</span>;
            case 'completed':
                return <span className="flex items-center gap-1 text-blue-600 bg-blue-50 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap"><CheckCircle size={14} /> Đã khám</span>;
            default:
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden min-h-full flex flex-col">
            ...
            <div className="px-6 py-5 border-b border-gray-100 bg-white flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Quản lý các cuộc hẹn của bạn</p>
                </div>
            ...
            <div className="p-6 flex-1">
                {filteredAppointments.length === 0 ? (
                    <div className="text-center py-10 text-gray-500">
                        ...
                        <p>{appointments.length === 0 ? "Bạn chưa có lịch khám nào." : "Không tìm thấy lịch khám phù hợp."}</p>
                        {appointments.length > 0 && (
                            <button onClick={() => setFilterStatus("ALL")} className="mt-2 text-sky-600 font-medium hover:underline">
                                Xóa bộ lọc
                            </button>
                        )}
                    </div>
                        ...
                        <div className="space-y-4">
                            {paginatedAppointments.map((apt) => {
                                ...
                                const specialtyName = 
                                    ...
                                    (typeof doctor.specialty === 'string' ? doctor.specialty : "") || // Case 3: Chỉ là string
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6 text-sm text-gray-600 mt-3">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span>Chuyên khoa: <span className="font-medium title-color">{specialtyName}</span></span>
                                                    </div>
                                                    
                                                    <div className="flex items-start gap-2 col-span-1 sm:col-span-2">
                                                        <AlertCircle size={16} className="title-color mt-0.5 shrink-0" />
                                                        <span className="text-gray-500 italic line-clamp-2">Ghi chú: {renderReason(apt.reason)}</span>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        {totalPages > 1 && (
                            <div className="flex justify-center items-center gap-4 mt-8 pt-4 border-t border-gray-100">
                                ...
                                <span className="text-sm font-medium text-gray-700">
                                    Trang <span className="title-color font-bold">{currentPage}</span> / {totalPages}
                                </span>
