...
export const initialMockUsers = [
  {
    ...
    role_id: MOCK_IDS.roles.admin,
    status: "active",
    profile_completed: true,
  },
  {
    id: MOCK_IDS.users.doctor1,
    name: "Lê Thị Mai (BS)",
    email: "mai.le@clinic.com",
    role_id: MOCK_IDS.roles.doctor,
    status: "active",
    profile_completed: true,
  },
  {
    id: MOCK_IDS.users.doctor2,
    name: "Nguyễn Văn Hùng (BS)",
  ...
  {
    id: MOCK_IDS.users.patient1,
    name: "Nguyễn Văn A (BN)",
    email: "a.nguyen@clinic.com",
    role_id: MOCK_IDS.roles.patient,
    status: "active",
    profile_completed: true,
  },
];
...
export const mockStatusData = [
  ...
  { status: "Đang chờ", percentage: 5, color: "bg-sky-500" },
];
...
export const mockActivity = [
  ...
  {
    time: "14:30",
    user: initialMockPatients[1].fullName,
    action: "vừa đặt lịch (MOCK) với BS. Hùng.",
    type: "sale",
  },
  {
    time: "12:00",
    user: initialMockDoctors[0].fullName,
    action: "đã hoàn thành lịch hẹn #A1.",
    type: "inventory",
  },
  {
    time: "09:00",
    user: "Admin",
    action: "đã thêm BS. Trần Ngọc Anh.",
    type: "user",
  },
  {
    time: "08:30",
    user: "Hệ thống",
    action: "Gửi email nhắc nhở lịch hẹn trong ngày.",
    type: "system",
  },
];
const totalFee = initialMockDoctors.reduce(
  (sum, doc) => sum + doc.consultation_fee,
  0
);
...
export const mockKPIs = [
  // Cập nhật lại logic dựa trên MOCK data mới nếu cần, nhưng giữ cấu trúc hiển thị
  {
    ...
    value: `${
      initialMockAppointments.filter(
        (a) => a.date === today && a.status === "confirmed"
      ).length
    }/${
      initialMockAppointments.filter((a) => a.date === today).length
    } (Confirmed)`,
    change: "+5.5%",
    period: "Hôm qua",
    isPositive: true,
