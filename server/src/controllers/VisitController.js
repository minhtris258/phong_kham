...
export const createVisit = async (req, res, next) => {
  ...
  try {
    ...
    if (next_visit_timeslot_id && mongoose.Types.ObjectId.isValid(next_visit_timeslot_id)) {
        const targetSlot = await Timeslot.findOne({ _id: next_visit_timeslot_id, status: "free" }).session(session);
        if (targetSlot) {
    ...
    if (patientProfile && patientProfile.user_id) {
        ...
        let visitBody = `Chẩn đoán: ${diagnosis || symptoms}. Tổng chi phí: ${totalAmount.toLocaleString('vi-VN')} đ.`;
    ...
    try {
        if (patientProfile && patientProfile.email) {
            ...
            const prescriptionListHtml = formattedPrescriptions.length > 0 
                ? formattedPrescriptions.map((p, index) => 
                    `<tr>
                        ...
                        <td style="padding: 8px; border: 1px solid #ddd;">
                            ${p.frequency || ''}
                            ${p.note ? `<br><i style="color: #666; font-size: 12px;">(${p.note})</i>` : ''}
                        </td>
            ...
            const serviceListHtml = billItems.length > 0
                ? billItems.map(s => `<li>${s.name}: ${s.price.toLocaleString('vi-VN')} đ</li>`).join('')
            ...
            const followUpHtml = followupInfo.scheduled 
                ? `<div style="margin-top: 15px; padding: 10px; background-color: #e8f5e9; border-left: 5px solid #28a745; color: #2e7d32;">
                    <strong>📅 LỊCH TÁI KHÁM:</strong> Ngày ${new Date(followupInfo.date).toLocaleDateString('vi-VN')}
                   </div>` 
                : '';
            ...
            await sendEmail({
                ...
                subject: `Kết Quả Khám Bệnh - ${new Date().toLocaleDateString('vi-VN')} - PK Tâm An`,
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; background-color: #ffffff;">
                        ...
                        <div style="text-align: center; border-bottom: 2px solid #007bff; padding-bottom: 15px; margin-bottom: 20px;">
                            ...
                            <p style="color: #555; margin: 5px 0;">Phòng Khám Đa Khoa Tâm An</p>
                        </div>
                        
                        <div style="display: flex; justify-content: space-between; margin-bottom: 20px; font-size: 14px;">
                            <div style="width: 48%;">
                                <p><b>Bệnh nhân:</b> ${patientProfile.fullName}</p>
                                <p><b>Ngày khám:</b> ${new Date().toLocaleDateString('vi-VN')}</p>
                            </div>
                            <div style="width: 48%; text-align: right;">
                                <p><b>Bác sĩ:</b> ${doc?.fullName || 'Bác sĩ'}</p>
                                <p><b>Chuyên khoa:</b> ${doc?.specialty || 'Đa khoa'}</p>
                            </div>
                        </div>
                        ...
                        <div style="margin-bottom: 20px;">
                            ...
                            <ul style="list-style-type: circle; padding-left: 20px; color: #555;">
                                <li>Phí khám tư vấn: ${consultationFee.toLocaleString('vi-VN')} đ</li>
                                ${serviceListHtml}
                            </ul>
                            <p style="font-size: 18px; text-align: right; margin-top: 10px;">
                                <b>Tổng cộng: <span style="color: #d9534f; font-size: 20px;">${totalAmount.toLocaleString('vi-VN')} đ</span></b>
                            </p>
                        </div>
            ...
            console.log(`✅ Email kết quả khám đã gửi tới: ${patientProfile.email}`);
        }
    } catch (emailErr) {
    ...
    if (e.message === "FORBIDDEN") return res.status(403).json({ error: "Không có quyền xử lý lịch này." });
...
export const updateVisit = async (req, res, next) => {
  ...
  try {
    ...
    const { 
        symptoms, diagnosis, notes, advice, 
        prescriptions, bill_items, next_visit_date 
    } = req.body;
    ...
    const patientProfile = await Patient.findById(v.patient_id).session(session);
    if (patientProfile && patientProfile.user_id) {
    ...
    if (isOwner && patientProfile && patientProfile.email) {
        try {
            ...
            const prescriptionHtml = v.prescriptions.length > 0 
                ? v.prescriptions.map((p, i) => `
                    <tr>
                        <td style="padding:5px;border:1px solid #ddd;text-align:center">${i+1}</td>
                        <td style="padding:5px;border:1px solid #ddd"><b>${p.drug}</b></td>
                        <td style="padding:5px;border:1px solid #ddd">${p.quantity} ${p.unit}</td>
                        <td style="padding:5px;border:1px solid #ddd">${p.dosage}</td>
                        <td style="padding:5px;border:1px solid #ddd">${p.frequency} <br><i>${p.note}</i></td>
                        <td style="padding:5px;border:1px solid #ddd">${p.duration}</td>
                    </tr>`).join('')
                : '<tr><td colspan="6">Không có thuốc</td></tr>';

            const serviceHtml = v.bill_items.map(s => `<li>${s.name}: ${s.price.toLocaleString('vi-VN')} đ</li>`).join('');
            
            await sendEmail({
                email: patientProfile.email,
                subject: `[CẬP NHẬT] Kết Quả Khám Bệnh - ${new Date().toLocaleDateString('vi-VN')}`,
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd; max-width: 800px; margin: 0 auto;">
                        ...
                        <p>Xin chào <b>${patientProfile.fullName}</b>, bác sĩ vừa chỉnh sửa thông tin khám bệnh của bạn:</p>
                        ...
                        <h3>Chi phí:</h3>
                        <ul>
                            <li>Phí khám: ${v.consultation_fee_snapshot.toLocaleString('vi-VN')} đ</li>
                            ${serviceHtml}
                        </ul>
                        <p style="text-align: right; font-weight: bold; font-size: 18px; color: #d9534f;">Tổng: ${v.total_amount.toLocaleString('vi-VN')} đ</p>
                    </div>
                `
            });
    ...
    await session.abortTransaction();
    if (e.message === "NOT_FOUND") return res.status(404).json({ error: "Không tìm thấy hồ sơ khám." });
    if (e.message === "FORBIDDEN") return res.status(403).json({ error: "Không đủ quyền truy cập." });
    next(e);
  } finally {
    session.endSession();
  }
...
export const deleteVisitAdmin = async (req, res, next) => {
  ...
  try {
    ...
    await session.commitTransaction();
    res.json({ message: "Đã xóa hồ sơ khám bệnh và khôi phục trạng thái lịch hẹn.", id });

  } catch (e) {
    await session.abortTransaction();
    if (e.message === "FORBIDDEN") return res.status(403).json({ error: "Chỉ Admin mới có quyền xóa." });
    if (e.message === "NOT_FOUND") return res.status(404).json({ error: "Không tìm thấy hồ sơ." });
    next(e);
  } finally {
    session.endSession();
  }
