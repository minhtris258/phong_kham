...
export const listOrGenerateSlots = async (req, res, next) => {
    try {
        ...
        let slots = await Timeslot.find({doctor_id: doctorId, date}).sort({start: 1}).lean();
        ...
        const dayDef = (schedule.weekly_schedule || []).find(d => d.day === dayName);
        ...
        if(ex) {
            if(ex.isDayOff) base = [];
                  ...
                  const removing = new Set((ex.removeSlot || []).map(r => `${r.start}-${r.end}`));
    ...
    for (const r of base) {
      ...
      while (t + schedule.slot_minutes <= end) {
        out.push({ doctor_id: doctorId, date, start: toHHMM(t), end: toHHMM(t + schedule.slot_minutes), status: "free" });
    ...
    slots = await Timeslot.find({ doctor_id: doctorId, date }).sort({ start: 1 }).lean();
